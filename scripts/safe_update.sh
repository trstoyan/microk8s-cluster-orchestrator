...
if [ -f .prod-server.pid ]; then
    echo "   make prod-restart     # Restart background server"
else
    echo "   make prod-start       # Start background server"
fi
echo "   OR"
echo "   sudo systemctl restart microk8s-orchestrator"
echo ""
echo "🧪 Verify Update:"
echo "   make prod-status      # Check server is running"
